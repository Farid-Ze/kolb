...
- **Konstruksi yang wajib dicakup**
  ...
  - Learning Flexibility Index (**LFI**): diturunkan dari koefisien Kendall’s W (lihat `docs/psychometrics_spec.md §2` and `backend/app/assessments/klsi_v4/logic.py::compute_learning_flexibility`).
...
- **Sumber norm**
  ...
  - Implementasi resmi berada di `backend/app/assessments/klsi_v4/logic.py::resolve_norm_groups` and `backend/app/engine/norms/factory.py`. Urutan wajib:
...
|----------|--------|--------|----------------|-----------------|--------------|--------|
| `/auth/register` | POST | Registrasi akun mahasiswa/mediator otomatis berdasarkan domain email. | `app.schemas.auth.UserCreate` | `app.schemas.auth.UserOut` | `backend/app/routers/auth.py::register` | Stable |
| `/auth/login` | POST | Menghasilkan JWT akses 60 menit. | Body `{"email": str, "password": str}` | `app.schemas.auth.Token` | `backend/app/routers/auth.py::login` | Stable |
...
| `/engine/sessions/{id}/submit_all` | POST | Kirim 12 item + 8 konteks sekaligus dan finalize atomik. | `app.schemas.session.SessionSubmissionPayload` | `{ "ok": true, "result": {...} }` | `backend/app/routers/engine.py` | Stable |
...
| `/score/raw` | POST | Preview skor tanpa persist (alat dosen/riset). | `app.schemas.score.ScorePreviewRequest` | `ScorePreviewResponse` | `backend/app/routers/score.py` | Stable |
...
- **Deploy**: saat membangun image/container, copy `docs/guides` ke lokasi yang sama dengan source code sehingga mount `/static/guides` tetap valid. Contoh Dockerfile snippet:
  ```dockerfile
  COPY docs/guides /backend/app/docs/guides
  ```
