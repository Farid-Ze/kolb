  ...
  return (
    <div className="space-y-4 rounded-xl border border-[var(--zen-border)] bg-[var(--zen-bg-elevated)] p-6 shadow-sm">
      ...
      {results.kiteCoordinates && (
        <section>
          <h3 className="text-sm font-semibold uppercase tracking-wide text-[var(--zen-text-muted)]">Kite Coordinates</h3>
          <dl className="mt-2 grid gap-3 sm:grid-cols-2">
            {Object.entries(results.kiteCoordinates).map(([dimension, value]) => (
              <div key={dimension} className="rounded-lg border border-[var(--zen-border)] bg-[var(--zen-bg)] px-3 py-2">
                <dt className="text-xs uppercase text-[var(--zen-text-muted)]">{dimension}</dt>
                <dd className="text-lg font-semibold text-[var(--zen-text)]">{value.toFixed(2)}</dd>
              </div>
            ))}
          </dl>
        </section>
