...
export const useVibrancyClass = (backgroundColor?: string): string => {
  ...
  return isLight ? 'text-white' : 'text-foreground';
};

/**
 * useAdaptiveTextColor - More sophisticated version dengan theme awareness
 * 
 * @param backgroundColor - Background color
 * @param theme - Current theme ('light' | 'dark')
 * @returns Object dengan color info
 * 
 * @example
...
export const useAdaptiveTextColor = (
  ...
  // Adjust untuk theme
  const className = vibrancy.isLight
  ...
  return {
    ...vibrancy,
    className,
    // CSS custom property untuk inline styles
    cssVar: vibrancy.textColor,
  };
};

/**
 * Helper: Check if a color combination meets WCAG standards
 * 
