...
def test_instrument_catalog_listing(client):
    ...
    entry = next((inst for inst in instruments if inst["code"] == "KLSI4" and inst["version"] == "4.0"), None)
...
def test_instrument_manifest_detail(client):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = client.get("/engine/instruments/KLSI4/4.0", headers=headers)
    assert response.status_code == 200, response.text
...
def test_instrument_locale_resources(client):
    _, token = _create_user()
    headers = {"Authorization": f"Bearer {token}"}
    response = client.get("/engine/instruments/KLSI4/4.0/resources/id", headers=headers)
    assert response.status_code == 200, response.text
    payload = response.json()
