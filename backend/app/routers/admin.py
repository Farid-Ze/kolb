...
async def import_norms(
    ...
    def _process_import():
        ...
        # norm_version is local var, need to use the one from outer scope or pass it
        # It's available in closure.
    ...
    return await run_in_threadpool(_process_import)


@router.get("/norms/cache-stats")
async def get_norm_cache_stats(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
...
async def get_external_norm_cache_stats(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    """
    require_mediator(current_user, AuthorizationMessages.MEDIATOR_EXTERNAL_CACHE_STATS_ONLY)
    ...
    # external_cache_stats is likely in-memory or simple, but let's wrap it just in case
    return await run_in_threadpool(external_cache_stats)
...
async def get_perf_metrics(
    ...
    """
    require_mediator(current_user, AuthorizationMessages.MEDIATOR_METRICS_ONLY)
    ...
    return await run_in_threadpool(_get_metrics)


@router.get("/instruments/{instrument_code}/pipelines")
async def list_instrument_pipelines(
    instrument_code: str,
    ...
    return await run_in_threadpool(pipeline_service.list_pipelines, db, instrument_code, instrument_version)


@router.post("/instruments/{instrument_code}/pipelines/{pipeline_id}/activate")
async def activate_instrument_pipeline(
    instrument_code: str,
    pipeline_id: int,
    instrument_version: str | None = None,
    db: Session = Depends(get_db),
...
async def clone_instrument_pipeline(
    instrument_code: str,
    pipeline_id: int,
    ...
    return await run_in_threadpool(_clone)


@router.delete("/instruments/{instrument_code}/pipelines/{pipeline_id}")
async def delete_instrument_pipeline(
    instrument_code: str,
    pipeline_id: int,
    instrument_version: str | None = None,
    db: Session = Depends(get_db),
