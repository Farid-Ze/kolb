...
class AssessmentTelemetryPayload(CamelModel):
    ...
    session_id: int = Field(gt=0, description="Assessment session identifier", serialization_alias="sessionId", validation_alias="sessionId")
...
class TimeOnPageEvent(CamelModel):
    ...
    session_id: int | None = Field(None, gt=0, serialization_alias="sessionId", validation_alias="sessionId")


class ItemChangedEvent(CamelModel):
    """Captures when a user changes their answer (rank) for an item."""
    session_id: int = Field(..., gt=0, serialization_alias="sessionId", validation_alias="sessionId")
