...
def test_team_rollup_single_session():
    ...
    u = User(full_name="U", email="u@example.com")
    ...
    st = LearningStyleType(style_name="Balancing", style_code="BAL", ACCE_min=6, ACCE_max=14, AERO_min=1, AERO_max=11, quadrant="Mid", description=None)
    ...
    db.add(LearningFlexibilityIndex(session_id=s.id, W_coefficient=0.5, LFI_score=0.5, LFI_percentile=None, flexibility_level=None))
    ...
    assert abs(roll.avg_lfi - 0.5) < 1e-9
    assert roll.style_counts.get("Balancing") == 1
    db.close()
