...
def create_study(payload: ResearchStudyCreate, db: Session = Depends(get_db), authorization: str | None = Header(default=None)):
...
def list_studies(db: Session = Depends(get_db), skip: int = Query(0, ge=0), limit: int = Query(50, ge=1, le=200), q: Optional[str] = Query(None)):
...
def update_study(study_id: int, payload: ResearchStudyUpdate, db: Session = Depends(get_db), authorization: str | None = Header(default=None)):
    user = _get_current_user(authorization, db)
    _require_mediator(user)
    study = db.query(ResearchStudy).filter(ResearchStudy.id == study_id).first()
    if not study:
...
def delete_study(study_id: int, db: Session = Depends(get_db), authorization: str | None = Header(default=None)):
    user = _get_current_user(authorization, db)
    _require_mediator(user)
    study = db.query(ResearchStudy).filter(ResearchStudy.id == study_id).first()
    if not study:
    ...
    if rel_count > 0 or val_count > 0:
        raise HTTPException(status_code=409, detail="Hapus bukti reliabilitas/validitas terlebih dahulu")
    db.delete(study); db.commit()
    return {"ok": True}


@router.post("/studies/{study_id}/reliability", response_model=dict)
def add_reliability(study_id: int, payload: ReliabilityCreate, db: Session = Depends(get_db), authorization: str | None = Header(default=None)):
    user = _get_current_user(authorization, db)
    _require_mediator(user)
    study = db.query(ResearchStudy).filter(ResearchStudy.id == study_id).first()
    if not study:
...
def add_validity(study_id: int, payload: ValidityCreate, db: Session = Depends(get_db), authorization: str | None = Header(default=None)):
    user = _get_current_user(authorization, db)
    _require_mediator(user)
    study = db.query(ResearchStudy).filter(ResearchStudy.id == study_id).first()
    if not study:
